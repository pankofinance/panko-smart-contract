...
import { abi as SwapLabsV3PoolDeployerABI } from "@pancakeswap/v3-core/artifacts/contracts/SwapLabsV3PoolDeployer.sol/SwapLabsV3PoolDeployer.json";
...
import { abi as SwapLabsStableSwapFactoryABI } from "@pancakeswap/stable-swap/artifacts/contracts/SwapLabsStableSwapFactory.sol/SwapLabsStableSwapFactory.json";

import { abi as SmartRouterABI } from "@pancakeswap/smart-router/artifacts/contracts/SmartRouter.sol/SmartRouter.json";

import { abi as SwapLabsTokenABI } from "@pancakeswap/masterchef-v3/artifacts/contracts/SwapLabsToken.sol/SwapLabsToken.json";
...
async function main() {
  ...
  const deployer = new ethers.Contract(
    core.V3PoolDeployer,
    SwapLabsV3PoolDeployerABI,
    owner
  );

  const pp = await import(
    `@pancakeswap/v3-periphery/deployed/${networkName}.json`
  );
  const nftDescriptorEx = new ethers.Contract(
    pp.NFTDescriptorEx,
    NFTDescriptorExABI,
    owner
  );
  ...
  const ss = await import(
    `@pancakeswap/stable-swap/deployed/${networkName}.json`
  );
  const stableSwapFactory = new ethers.Contract(
    ss.StableSwapFactory,
    SwapLabsStableSwapFactoryABI,
    owner
  );

  const router = await import(
    `@pancakeswap/smart-router/deployed/${networkName}.json`
  );
  ...
  const mancakeToken = new ethers.Contract(
    mc.SwapLabsToken,
    SwapLabsTokenABI,
    owner
  );
  const masterchef = new ethers.Contract(mc.MasterChef, MasterChefABI, owner);
  const masterchefv2 = new ethers.Contract(
    mc.MasterChefV2,
    MasterChefV2ABI,
    owner
  );
  ...
  const lm = await import(
    `@pancakeswap/v3-lm-pool/deployed/${networkName}.json`
  );

  const vote = await import(`@pancakeswap/voter/deployed/${networkName}.json`);
  const veSwapLabs = new ethers.Contract(vote.VESwapLabs, VESwapLabsABI, owner);
  const gaugeVoting = new ethers.Contract(
    vote.GaugeVoting,
    GaugeVotingABI,
    owner
  );
  const gaugeVotingAdminUtil = new ethers.Contract(
    vote.GaugeVotingAdminUtil,
    GaugeVotingAdminUtilABI,
    owner
  );
  ...
  const farm = await import(
    `@pancakeswap/v3-farm/deployed/${networkName}.json`
  );
  const farmBooster = new ethers.Contract(
    farm.FarmBooster,
    FarmBoosterABI,
    owner
  );
  ...
  const masterchefv3receiver = new ethers.Contract(
    fee.MasterChefV3Receiver,
    MasterChefV3ReceiverABI,
    owner
  );
  ...
  await tx.wait(5);
  console.log("SwapLabsToken setOwner:", tx.hash);
  ...
  tx = await veSwapLabs.transferOwnership(ADMIN);
  await tx.wait(5);
  console.log("VESwapLabs setOwner:", tx.hash);
