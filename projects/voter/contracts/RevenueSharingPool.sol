...
contract RevenueSharingPool is Ownable, ReentrancyGuard {
    ...
    /// @dev VECake supply at week bounds
    ...
    /// @notice Get VECake balance of "_user" at "_timstamp"
    ...
    function balanceOfAt(address _user, uint256 _timestamp) external view returns (uint256) {
        (address cakePoolProxy, , , , , ) = IVECake(VECake).userInfo(_user);

        if (cakePoolProxy != address(0)) {
            return _balanceOfAt(_user, _timestamp) + _balanceOfAt(cakePoolProxy, _timestamp);
        } else {
    ...
    function balanceOfAtProxy(address _user, uint256 _timestamp) external view returns (uint256) {
        (address cakePoolProxy, , , , , ) = IVECake(VECake).userInfo(_user);
        if (cakePoolProxy != address(0)) {
            return _balanceOfAt(cakePoolProxy, _timestamp);
        }
    }
    ...
    function _balanceOfAt(address _user, uint256 _timestamp) internal view returns (uint256) {
        uint256 _maxUserEpoch = IVECake(VECake).userPointEpoch(_user);
        if (_maxUserEpoch == 0) {
            return 0;
        }

        uint256 _epoch = _findTimestampUserEpoch(_user, _timestamp, _maxUserEpoch);
        Point memory _point = IVECake(VECake).userPointHistory(_user, _epoch);
    ...
    function _checkpointTotalSupply() internal {
        ...
        for (uint256 _i = 0; _i < 52; _i++) {
            if (_weekCursor > _roundedTimestamp) {
                ...
                Point memory _point = IVECake(VECake).pointHistory(_epoch);
                int128 _timeDelta = 0;
    ...
    /// @notice Update VECake total supply checkpoint
    ...
    function _claim(address _user, address _recipient, uint256 _maxClaimTimestamp) internal returns (uint256) {
        ...
        Point memory _userPoint = IVECake(VECake).userPointHistory(_user, _userEpoch);
        ...
        for (uint256 _i = 0; _i < 52; _i++) {
            ...
            if (_userWeekCursor >= _userPoint.timestamp && _userEpoch <= _maxUserEpoch) {
                ...
                if (_userEpoch > _maxUserEpoch) {
                    ...
                    _userPoint = IVECake(VECake).userPointHistory(_user, _userEpoch);
                }
            } else {
    ...
    function getRecipient(address _user) public view returns (address _recipient) {
        ...
        bool isCakePoolProxy = IVECake(VECake).isCakePoolProxy(_user);
        if (isCakePoolProxy) {
            _recipient = IProxyForCakePool(_user).cakePoolUser();
        }
    ...
    function _claimWithCakePoolProxy(address _user, address _recipient) internal returns (uint256) {
        ...
        (address cakePoolProxy, , , , , ) = IVECake(VECake).userInfo(_user);
        if (cakePoolProxy != address(0)) {
            _total += _claim(cakePoolProxy, _recipient, _lastTokenTimestamp);
        }
        if (_total != 0) {
    ...
    function claimMany(address[] calldata _users) external nonReentrant onlyLive returns (bool) {
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            (address cakePoolProxy, , , , , ) = IVECake(VECake).userInfo(_user);
            if (cakePoolProxy != address(0)) {
                _amount += _claim(cakePoolProxy, _recipient, _lastTokenTimestamp);
            }
            if (_amount != 0) {
                rewardToken.safeTransfer(_recipient, _amount);
    ...
    function _findTimestampEpoch(uint256 _timestamp) internal view returns (uint256) {
        uint256 _min = 0;
        uint256 _max = IVECake(VECake).epoch();
        ...
        for (uint256 i = 0; i < 128; i++) {
            ...
            Point memory _point = IVECake(VECake).pointHistory(_mid);
    ...
    function _findTimestampUserEpoch(
        ...
        for (uint256 i = 0; i < 128; i++) {
            ...
            uint256 _mid = (_min + _max + 1) / 2;
            Point memory _point = IVECake(VECake).userPointHistory(_user, _mid);
            if (_point.timestamp <= _timestamp) {
                _min = _mid;
            } else {
                _max = _mid - 1;
    ...
    /// @dev In case some partners integrated our VECake , but did not write claim logic for revenue sharing pool. So owner can set recipient for them , partners still can get reward.
    /// @dev If the user address is not EOA, You can set recipient once , then owner will lose permission to set recipient for the user
