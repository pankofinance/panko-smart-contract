...
import {
  advanceBlock,
  advanceBlockTo,
} from "@openzeppelin/test-helpers/src/time";
import { artifacts, contract, ethers } from "hardhat";
import { parseEther } from "ethers/lib/utils";
import { BigNumber } from "ethers";
import { assert, expect } from "chai";

const PancakeStableSwapFactory = artifacts.require(
  "stable-swap/contracts/SwapLabsStableSwapFactory.sol"
);
const PancakeStableSwapLPFactory = artifacts.require(
  "stable-swap/contracts/SwapLabsStableSwapLPFactory.sol"
);
const PancakeStableSwapTwoPoolDeployer = artifacts.require(
  "stable-swap/contracts/SwapLabsStableSwapTwoPoolDeployer.sol"
);
const PancakeStableSwapThreePoolDeployer = artifacts.require(
  "stable-swap/contracts/SwapLabsStableSwapThreePoolDeployer.sol"
);
const PancakeStableSwapTwoPool = artifacts.require(
  "stable-swap/contracts/SwapLabsStableSwapTwoPool.sol"
);
const PancakeStableSwapThreePool = artifacts.require(
  "stable-swap/contracts/SwapLabsStableSwapThreePool.sol"
);
const PancakeStableSwapTwoPoolInfo = artifacts.require(
  "stable-swap/contracts/utils/PancakeStableSwapTwoPoolInfo.sol"
);
const PancakeStableSwapThreePoolInfo = artifacts.require(
  "stable-swap/contracts/utils/PancakeStableSwapThreePoolInfo.sol"
);
const PancakeStableSwapInfo = artifacts.require(
  "stable-swap/contracts/utils/SwapLabsStableSwapInfo.sol"
);
const LPToken = artifacts.require(
  "stable-swap/contracts/SwapLabsStableSwapLP.sol"
);
const Token = artifacts.require("stable-swap/contracts/test/Token.sol");
const FeeOnTransferToken = artifacts.require(
  "exchange-protocol/contracts/test/FeeOnTransferToken.sol"
);
const PancakeFactory = artifacts.require(
  "exchange-protocol/contracts/SwapLabsFactory.sol"
);
const PancakePair = artifacts.require(
  "exchange-protocol/contracts/SwapLabsPair.sol"
);
const PancakeRouter = artifacts.require(
  "exchange-protocol/contracts/SwapLabsRouter.sol"
);
const WrappedBNB = artifacts.require(
  "exchange-protocol/contracts/libraries/WBNB.sol"
);
const StableSwapRouterHelper = artifacts.require(
  "./libraries/StableSwapRouterHelper.sol"
);
...
contract("SmartRouter", ([admin, bob, carol]) => {
  ...
  before(async () => {
    ...
    FeeToken = await FeeOnTransferToken.new(
    ...
    stableSwap2PoolDeployer = await PancakeStableSwapTwoPoolDeployer.new({
      from: admin,
    });
    stableSwap3PoolDeployer = await PancakeStableSwapThreePoolDeployer.new({
      from: admin,
    });
    ...
    stableSwap2PoolInfo = await PancakeStableSwapTwoPoolInfo.new({
      from: admin,
    });
    stableSwap3PoolInfo = await PancakeStableSwapThreePoolInfo.new({
      from: admin,
    });
    stableSwapPoolInfo = await PancakeStableSwapInfo.new(
      stableSwap2PoolInfo.address,
      stableSwap3PoolInfo.address,
      {
        from: admin,
      }
    );

    await stableSwapLPFactory.transferOwnership(stableSwapFactory.address, {
      from: admin,
    });
    await stableSwap2PoolDeployer.transferOwnership(stableSwapFactory.address, {
      from: admin,
    });
    await stableSwap3PoolDeployer.transferOwnership(stableSwapFactory.address, {
      from: admin,
    });

    await stableSwapFactory.createSwapPair(
      BUSD.address,
      USDC.address,
      A,
      Fee,
    ...
    stable2PoolInfo = await stableSwapFactory.getPairInfo(
      BUSD.address,
      USDC.address
    );
    stable2Pool_BUSD_USDC = await PancakeStableSwapTwoPool.at(
      stable2PoolInfo.swapContract
    );
    ...
    stable2PoolInfo_WBNB_BUSD = await stableSwapFactory.getPairInfo(
      WBNB.address,
      BUSD.address
    );
    stable2Pool_WBNB_BUSD = await PancakeStableSwapTwoPool.at(
      stable2PoolInfo_WBNB_BUSD.swapContract
    );
    stable2Pool_LP_WBNB_BUSD = await LPToken.at(
      stable2PoolInfo_WBNB_BUSD.LPContract
    );
    stable2Pool_WBNB_BUSD_token0 = await Token.at(
      stable2PoolInfo_WBNB_BUSD.token0
    );
    stable2Pool_WBNB_BUSD_token1 = await Token.at(
      stable2PoolInfo_WBNB_BUSD.token1
    );
    await stableSwapFactory.createThreePoolPair(
      BUSD.address,
      USDC.address,
      USDT.address,
      A,
    ...
    stable3PoolInfo = await stableSwapFactory.getThreePoolPairInfo(
      BUSD.address,
      USDC.address
    );
    stable3Pool_BUSD_USDC_USDT = await PancakeStableSwapThreePool.at(
      stable3PoolInfo.swapContract
    );
    stable3Pool_LP_BUSD_USDC_USDT = await LPToken.at(
      stable3PoolInfo.LPContract
    );
    ...
    await BUSD.approve(stable2Pool_BUSD_USDC.address, parseEther("1000000"), {
      from: bob,
    });
    await USDC.approve(stable2Pool_BUSD_USDC.address, parseEther("1000000"), {
      from: bob,
    });

    await expectRevert(
      stable2Pool_BUSD_USDC.add_liquidity([0, parseEther("1")], 0, {
        from: bob,
      }),
    ...
    await expectRevert(
      stable2Pool_BUSD_USDC.add_liquidity([parseEther("1"), 0], 0, {
        from: bob,
      }),
      "Initial deposit requires all coins"
    );
    ...
    await WBNB.approve(stable2Pool_WBNB_BUSD.address, parseEther("100"), {
      from: bob,
    });
    await BUSD.approve(stable2Pool_WBNB_BUSD.address, parseEther("20000"), {
      from: bob,
    });
    ...
    await BUSD.approve(
      stable3Pool_BUSD_USDC_USDT.address,
      parseEther("1000000"),
      { from: bob }
    );
    await USDC.approve(
    ...
    await expectRevert(
      stable3Pool_BUSD_USDC_USDT.add_liquidity([0, parseEther("1"), 0], 0, {
        from: bob,
      }),
      "Initial deposit requires all coins"
    );
    ...
    await expectRevert(
      stable3Pool_BUSD_USDC_USDT.add_liquidity([parseEther("1"), 0, 0], 0, {
        from: bob,
      }),
      "Initial deposit requires all coins"
    );
    ...
    await expectRevert(
      stable3Pool_BUSD_USDC_USDT.add_liquidity([0, 0, parseEther("1")], 0, {
        from: bob,
      }),
      "Initial deposit requires all coins"
    );
  ...
  describe("Check setups and info", () => {
    it("Check Stable Swap", async () => {
      ...
      assert.equal(
        stable2PoolInfo.LPContract,
        stable2Pool_LP_BUSD_USDC.address
      );
      ...
      assert.equal(
        stable3PoolInfo.LPContract,
        stable3Pool_LP_BUSD_USDC_USDT.address
      );

      await expectRevert(
        stable2Pool_BUSD_USDC.add_liquidity(
  ...
  describe("User swap via SmartRouter", () => {
    it("Carol exact output swap BUSD -> USDC on stable 2pool & exact output swap USDC -> 100 USDT on stable 3pool", async () => {
      ...
      const exactOnputData = smartRouter.contract.methods
        ...
        .encodeABI();
      const result = await smartRouter.multicall(deadline, [exactOnputData], {
        from: carol,
      });

      const usdtBalanceAfter = await USDT.balanceOf(carol);
      assert.equal(
        usdtBalanceAfter.sub(usdtBalanceBefore).toString(),
    ...
    it("Carol exact input swap 100 BUSD -> USDC on stable 2pool & exact output swap USDC -> 100 USDT on stable 3pool", async () => {
      ...
      const exactOnput3PoolData = smartRouter.contract.methods
        ...
        .encodeABI();
      const result = await smartRouter.multicall(
        deadline,
        [exactInput2PoolData, exactOnput3PoolData],
        { from: carol }
      );
      ...
      const usdtBalanceAfter = await USDT.balanceOf(carol);
      assert.equal(
        busdBalanceBefore.sub(busdBalanceAfter).toString(),
        parseEther("100").toString()
      );
      assert.equal(
        usdtBalanceAfter.sub(usdtBalanceBefore).toString(),
    ...
    it("Carol exact output swap BUSD -> 100 USDC on stable 2pool & exact output swap BUSD -> 100 USDC on stable 3pool", async () => {
      ...
      const result = await smartRouter.multicall(
        deadline,
        [exactOutput2PoolData, exactOnput3PoolData],
        { from: carol }
      );

      const usdcBalanceAfter = await USDC.balanceOf(carol);
      assert.equal(
        usdcBalanceAfter.sub(usdcBalanceBefore).toString(),
    ...
    it("Carol exact input swap 100 BUSD -> USDC on stable 2pool & exact output swap BUSD -> USDT -> 100 USDC on stable 3pool", async () => {
      ...
      const exactOnput3PoolData = smartRouter.contract.methods
        .exactOutputStableSwap(
          [BUSD.address, USDT.address, USDC.address],
          [3, 3],
          parseEther("100"),
          parseEther("105")
        )
    ...
    it("Carol exact output swap BUSD -> 1 BNB on stable 2pool", async () => {
      ...
      const exactOutput2PoolData = smartRouter.contract.methods
        ...
        .encodeABI();
      const unwrapWETH9Data = smartRouter.contract.methods
        .unwrapWETH9(parseEther("1"))
        .encodeABI();
      const result = await smartRouter.multicall(
        deadline,
        [exactOutput2PoolData, unwrapWETH9Data],
        {
          from: carol,
          gasPrice: 339817,
        }
      ...
      assert.equal(
        ...
        parseEther("1").sub(gasCost).toString()
      );
      assert.equal(
        (await WBNB.balanceOf(carol)).toString(),
    ...
    it("Carol exact output swap BNB -> 100 BUSD on stable 2pool", async () => {
      ...
      assert.equal(
        busdBalanceAfter.sub(busdBalanceBefore).toString(),
        parseEther("100").toString()
      );
  ...
  describe("Owner operation", () => {
    it("Update StableSwapRouter", async () => {
      ...
      const result = await smartRouter.setStableSwap(
        constants.ZERO_ADDRESS,
        constants.ZERO_ADDRESS,
        { from: admin }
      );
