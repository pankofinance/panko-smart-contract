...
import { GainV3PoolDeployer } from '../typechain-types/contracts/GainV3PoolDeployer'
...
describe('PancakeV3Factory', () => {
  ...
  let deployer: GainV3PoolDeployer
  let factory: PancakeV3Factory
  let poolBytecode: string
  const fixture = async () => {
    const deployerFactory = await ethers.getContractFactory('GainV3PoolDeployer')
    const factoryFactory = await ethers.getContractFactory('PancakeV3Factory')
    const deployer_ = (await deployerFactory.deploy()) as GainV3PoolDeployer
    ...
    return [factory_, deployer_] as [PancakeV3Factory, GainV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('GainV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('GainV3Pool')
