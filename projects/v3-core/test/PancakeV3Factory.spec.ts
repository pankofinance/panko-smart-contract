...
import { SwapLabsV3PoolDeployer } from '../typechain-types/contracts/SwapLabsV3PoolDeployer'
...
describe('PancakeV3Factory', () => {
  ...
  let deployer: SwapLabsV3PoolDeployer
  let factory: PancakeV3Factory
  let poolBytecode: string
  const fixture = async () => {
    const deployerFactory = await ethers.getContractFactory('SwapLabsV3PoolDeployer')
    const factoryFactory = await ethers.getContractFactory('PancakeV3Factory')
    const deployer_ = (await deployerFactory.deploy()) as SwapLabsV3PoolDeployer
    ...
    return [factory_, deployer_] as [PancakeV3Factory, SwapLabsV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('SwapLabsV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('SwapLabsV3Pool')
